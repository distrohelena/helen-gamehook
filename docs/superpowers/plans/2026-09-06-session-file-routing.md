...
- [ ] Add a hooked-child regression that changes process CWD to the protected directory, writes/mutates its relative filename, checks unchanged original and updated overlay, and restores CWD via RAII. Correct service/adapter request normalization to use native current-directory semantics consistently for classification and native dispatch; the constructor base may still resolve configured relative declarations. Batman imports GetCurrentDirectoryW and SetCurrentDirectoryW, so startup-base matching alone is not a valid runtime contract. Modify FileWriteRoutingService/FileApiHookSet as needed for this integration boundary.
...
- [ ] Also compose a distinctly labeled routing-plus-no-save experiment using the existing `Prepare-NoSaveProbe.ps1` and `NoSaveProbe.targets` with a second fresh native output root. Do not modify normal session source. This candidate verifies automatic engine writes only; it intentionally returns NotApplied and cannot verify explicit saves. Keep both candidates uninstalled and their provenance separate. Detailed parameter map is in this plan's scratch `candidate-notes.md`.

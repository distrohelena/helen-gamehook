...
/// <summary>
/// Applies the direct MainMenu.MainV2 audio subtitle-size patch to one Unreal frontend package.
/// </summary>
internal static class MainMenuAudioSubtitleSizePackagePatcher
{
    /// <summary>
    ...
    /// <summary>
    /// Patches one frontend package so the main-menu audio screen exposes the dedicated subtitle-size row.
    /// </summary>
    ...
    public static void PatchPackage(string packagePath, string outputPath, string prototypeGfxPath)
    {
        ...
        byte[] patchedPayload = MainMenuAudioSubtitleSizeGfxPatcher.Patch(originalPayload, prototypeGfxPath);

        string tempDirectory = Path.Combine(Path.GetTempPath(), "batman-mainv2-audio-subtitle-size-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDirectory);

        try
        {
            string payloadPath = Path.Combine(tempDirectory, "MainV2-subtitle-size.gfx");
        ...
        string manifestDirectory)
    {
        ...
        {
            Name = "MainMenu MainV2 audio subtitle-size patch",
            Patches =
            [
        ...
        manifest.Initialize(Path.Combine(manifestDirectory, "generated-mainv2-audio-subtitle-size.manifest.jsonc"));
        return manifest;
    }
}

...
$output = Join-Path $repo ('output\session-protocol-' + [Guid]::NewGuid().ToString('N'))
...
$arguments = @('/nologo','/std:c++20','/EHsc','/MD','/W4','/WX','/DUNICODE','/D_UNICODE','/DNOMINMAX',
    ...
    "$PSScriptRoot\SessionProtocolTests.cpp", "$repo\HelenRuntime\BatmanGraphicsSessionService.cpp", "/Fo$output\", "/Fe$output\SessionProtocolTests.exe", '/link', '/LTCG', $RuntimeLibraryPath,
...
& $tool -FilePath "$output\SessionProtocolTests.exe" -Arguments @()

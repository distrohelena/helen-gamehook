...
$arguments = @('/nologo','/std:c++20','/EHsc','/MD','/W4','/WX','/DUNICODE','/D_UNICODE','/DNOMINMAX',
    ...
    (Join-Path $PSScriptRoot 'NoSaveSessionTests.cpp'), "/Fo$output\", "/Fe$output\NoSaveSessionTests.exe", '/link', $RuntimeLibraryPath,
...
& $tool -FilePath "$output\NoSaveSessionTests.exe" -Arguments @("$output\fixtures")

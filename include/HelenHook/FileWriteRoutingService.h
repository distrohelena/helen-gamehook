...
#include <HelenHook/FileWriteRoutingDiagnostics.h>
...
class FileWriteRoutingService {
  public:
    /** @brief Compatibility alias exposing immutable active-route diagnostics. */
    using RouteDiagnostics = routing_detail::RouteDiagnostics;
     ...
     */
    bool TryNormalizeRequestPath(const std::filesystem::path &path, std::filesystem::path &normalized_path) const;
     ...
     */
    HANDLE Open(const std::filesystem::path &path, DWORD access, DWORD share, LPSECURITY_ATTRIBUTES security_attributes, DWORD disposition,
     ...
     */
    BOOL Replace(const std::filesystem::path &replaced_path, const std::filesystem::path &replacement_path, LPCWSTR backup_file_name,
     ...
     */
    std::unique_ptr<FileWriteRoutingTransaction> BeginTrustedWrite(const std::vector<std::filesystem::path> &original_paths, DWORD &error);
  ...
  private:
    ...
    bool TryNormalizeRequestPathUnlocked(const std::filesystem::path &path, std::filesystem::path &normalized_path) const;
};
} // namespace helen

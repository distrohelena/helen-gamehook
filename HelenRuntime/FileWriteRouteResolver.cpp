...
namespace helen {
FileWriteRouteResolver::FileWriteRouteResolver(std::filesystem::path game_root, std::filesystem::path documents_root)
    : game_root_(std::move(game_root)), documents_root_(std::move(documents_root)) {
}

bool FileWriteRouteResolver::TryResolve(const std::vector<FileWriteRouteDefinition> &definitions, std::vector<FileWriteRoute> &routes,
    ...
    for (const FileWriteRouteDefinition &definition : definitions) {
        if (definition.Id.empty() || !route_ids.insert(FoldAscii(definition.Id)).second || !IsSafeRelativePath(definition.Path)) {
            failure_reason = "Route declaration has an empty, duplicate, or unsafe identifier/path.";
            routes.clear();
            return false;
        }

        const std::filesystem::path root = definition.Root == "game"        ? normalized_game_root
                                           : definition.Root == "documents" ? normalized_documents_root
                                                                            : std::filesystem::path();
        if (root.empty()) {
            failure_reason = "Route declaration selected an unsupported root.";
            routes.clear();

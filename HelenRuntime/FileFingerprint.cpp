...
namespace helen
{
     ...
     */
    FileFingerprint FileFingerprint::FromPath(const std::filesystem::path& file_path)
    {
        if (file_path.empty())
        {
            throw std::invalid_argument("File fingerprinting requires a non-empty file path.");
        }

        if (!IsRegularFile(file_path))
        {
            throw std::invalid_argument("File fingerprinting requires an existing regular file.");
        }
        ...
        if (error_code)
        {
            throw std::runtime_error("File fingerprinting failed to read the file size.");
        }

        std::string digest;
        if (!TryComputeSha256(file_path, digest))
        {
            throw std::runtime_error("File fingerprinting failed to compute the SHA-256 digest.");
        }
